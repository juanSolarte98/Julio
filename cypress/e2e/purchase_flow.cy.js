  ...
  context('purchase flow', () => {        
      ...
      it.only('PF-001: Verify purchase flow for guest users with home delivery method', () => {
        ...
        cy.getInitialCartCount().then((initialCount) => {
          ...
          cy.get('select[name="custom_attributes[state_custom]"]').then($select => {
            ...
            cy.get('select[name="custom_attributes[state_custom]"]').select(randomValue);
          });
          cy.wait(2000)
          //Selección de Ciudad         
          cy.get('select[name="custom_attributes[city_custom]"]').then($select => {
            // Obtiene todas las opciones excepto la primera (asumiendo que es un placeholder)
            const $options = $select.find('option').not(':first-child');
            // Calcula un índice aleatorio basado en la cantidad de opciones
            const randomIndex = Math.floor(Math.random() * $options.length);
            ...
            const randomValue = $options.eq(randomIndex).val();
            // Selecciona la opción en el select
            cy.get('select[name="custom_attributes[city_custom]"]').select(randomValue);
          });

          cy.wait(2000)
          //Selección de Zona
          cy.get('select[name="custom_attributes[zone_custom]"]').then($select => {
            // Obtiene todas las opciones excepto la primera (asumiendo que es un placeholder)
            const $options = $select.find('option').not(':first-child');
            // Calcula un índice aleatorio basado en la cantidad de opciones
            const randomIndex = Math.floor(Math.random() * $options.length);
            ...
            const randomValue = $options.eq(randomIndex).val();
            // Selecciona la opción en el select
            cy.get('select[name="custom_attributes[zone_custom]"]').select(randomValue);
          });
